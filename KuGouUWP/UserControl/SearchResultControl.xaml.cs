...
using static KuGouUWP.Class.Model.SearchResultModel;
    ...
    public sealed partial class SearchResultControl : UserControl
    {
        ...
        public static async Task<ObservableCollection<Class.Model.SearchResultModel.Song>> GetSongResult(string keyword, int page)
        {
            try
            {
                ...
                var jsondata= await httpclient.GetStringAsync("http://mobilecdn.kugou.com/api/v3/search/song?pagesize=20&sver=2&page=" + page + "&version=8150&keyword=" + keyword);
        ...
        public static async Task<ObservableCollection<Class.Model.SearchResultModel.MV>> GetMVResult(string keyword, int page)
        {
            ...
            {
                var httpclient = new System.Net.Http.HttpClient();
                var jsondata= await httpclient.GetStringAsync("http://mobilecdn.kugou.com/api/v3/search/mv?pagesize=20&sver=2&page=" + page + "&version=8150&keyword=" + keyword);
        ...
        {
            if(SongResultList.SelectedItems!=null)
            {
                SonglistLoadProgress.IsActive = true;
                switch (type)
                {
                    ...
                    case UserControlClass.SongMultipleBox.BtnType.SelectAll:
                        if (SongResultList.SelectedItems.Count == 0)
                        {
                            SongResultList.SelectAll();
                        }
                        else
                        {
                            SongResultList.SelectedItems.Clear();
                        }
                        break;
                    default:
                        break;
