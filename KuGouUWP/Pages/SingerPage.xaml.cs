    ...
    public sealed partial class SingerPage : Page
    {
        ...
        private void AlbumList_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (list.SelectedItem != null)
            {
                ...
                Frame.Navigate(typeof(Pages.AlbumPage), data.albumid);
                list.SelectedIndex = -1;
            }
        }
        ...
        public class SongListData
        {
            ...
            public async Task LoadData()
            {
                try
                {
                    page = page + 1;
                    var httpclient = new System.Net.Http.HttpClient();
                    var json = await httpclient.GetStringAsync("http://mobilecdn.kugou.com/api/v3/singer/song?pagesize=20&singerid=" + singerid + "&plat=0&page=" + page.ToString() + "&sorttype=" + ((int)sort).ToString() + "&version=8150");
        ...
        public class AlbumListData
        {
            ...
            public async Task LoadData()
            {
                try
                {
                    page = page + 1;
                    var httpclient = new System.Net.Http.HttpClient();
                    var json = await httpclient.GetStringAsync("http://mobilecdn.kugou.com/api/v3/singer/album?pagesize=20&singerid=" + singerid + "&plat=0&page=" + page.ToString() + "&version=8150");
        ...
        private async void SongListAdd_Click(object sender, RoutedEventArgs e)
        {
            ...
            var songdata = btn.DataContext as SongListData.SongData;
            if (songdata != null)
            {
                LoadProcess.IsActive = true;
